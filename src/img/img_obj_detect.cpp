...
private:
    ...
    //オブジェクト数カウント用変数
    //例: 1個目のオブジェクト(キー番号: 0)
    //mapから見たオブジェクト位置 objs_pos_[0][0] = x[m], objs_pos_[0][1]= y[m], objs_pos_[0][2] = z[m]
    //base_linkから見たオブジェクト位置 objs_base_link_pos_[0][0] = x[m], objs_base_link_pos_[0][1]= y[m], objs_base_link_pos_[0][2] = z[m]
    //投票数 objs_poll_[0] = 1～?
    vector< vector<float> > objs_pos_; //mapから見たオブジェクト位置
...
public:
    ...
    ObjDetect(){
        ...
        sub_smpc_cam_ = nh_.subscribe("/camera/depth_registered/points", 1, &ObjDetect::subf_smpc_cam, this);
