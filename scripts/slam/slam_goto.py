...
if __name__ == '__main__':
    ...
    if rospy.get_param('/param/dbg/sm/flow') == 0 and rospy.get_param('/param/dbg/speech/onlyspeech') == 0:
        commonf_speech_multi('移動します。')

        rospy.Subscriber("/move_base/cmd_vel", Twist, subf_move_base_cmd_vel)

        target_x = float(rospy.get_param('/param/slam/goal/pos/x'))
        target_y = float(rospy.get_param('/param/slam/goal/pos/y'))
        target_yaw = float(rospy.get_param('/param/slam/goal/pos/yaw'))
